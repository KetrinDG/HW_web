...
class Search(Command):
    def processing(self, data_container: AddressBot):
        category_completer = WordCompleter(
            ["name", "phone", "email", "address", "birthday"]
        )
        category = (
            prompt(
                "Enter a search category (name/phone/email/address/birthday) : ",
        ...
        search_parameter = (
            input("Enter the search parameter: ").lower().replace(" ", "")
        )
...
class Edit(Command):
    def processing(self, data_container: AddressBot):
        ...
        new_category = (
            prompt(
                "Enter a category (name/phone/address/email/birthday): ",
                completer=category_completer,
            )
            .strip()
            .lower()
...
class Delete(Command):
    def processing(self, data_container: AddressBot):
        contact_info_to_remove = (
            input("Enter any data about contact that you want to delete: ")
            .strip()
            .lower()
        )
        data_container.book.save()
    ...
    def validate(self, document) -> None:
        ...
        if not days.isdigit():
            raise ValidationError(
                message="Days should be an integer", cursor_position=0
            )
        return days
