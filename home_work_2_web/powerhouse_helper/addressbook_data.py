...
class Phone(Field):
    def __init__(self):
        self.value = prompt(
            "Phone(-s) (correct format: +380.........): ", validator=ValidatePhone()
        )
        ...
        for phone in value[1:]:
            if re.match(r"[+380][0-9]{11}", phone):
                ...
                raise ValidationError(
                    message="Incorrect input! The correct number format is +380.........",
                    cursor_position=0,
                )
...
class ValidateEmail(Validator):
    def validate(self, document) -> None:
        ...
        if (
            not re.match(
                r"[A-Z|a-z][A-Z|a-z|0-9|_|.]{1,}@[a-z|A-Z|0-9|_]+\.[A-Za-z]{2,}", email
            )
            and not len(email) == 0
        ):
...
class ValidateBirthday(Validator):
    def validate(self, document) -> None:
        ...
        if re.match(r"[0-9]{2}[-]{1}[0-9]{2}[-]{1}[0-9]{4}", birth):
            ...
            raise ValidationError(
                message="Incorrect format! You should enter birthday in format: day-month-year",
                cursor_position=0,
            )
