...
async def searcher(part_to_search: str, db: Session):
    ...
    for contact in contacts_all:
        if (
            part_to_search.capitalize() in contact.name.capitalize()
            and contact not in contact_list
        ):
            contact_list.append(contact)
        if (
            part_to_search.capitalize() in contact.surname.capitalize()
            and contact not in contact_list
        ):
            contact_list.append(contact)
        if (
            part_to_search.capitalize() in contact.email.capitalize()
            and contact not in contact_list
        ):
            contact_list.append(contact)
